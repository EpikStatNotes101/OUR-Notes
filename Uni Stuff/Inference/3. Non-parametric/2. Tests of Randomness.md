...
>***Proof*** For $r$ even, there must be the same number of runs of both types. Thus the only possible values of $r_1$ and $r_2$ are $r_1=r_2=r/2$, and $f_{R_1,R_2}(r_1,r_2)$ is summed over this pair. If $r_1=r_2\pm 1$, $r$ is odd. In this case, $f_{R_1,R_2}(r_1,r_2)$ is summed over the two pairs of values $r_1=(r-1)/2$ and $r_2=()$
