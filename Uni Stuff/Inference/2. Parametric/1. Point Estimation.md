...
>(If $E_{00}(v_0^2)=0$ then $P_{00}(v_0=0)=1$, and hence $E_{00}(T_0v_0)=0$)
>$$\therefore V_{00}(T_0+v_0)=V_{00}(T)-\frac{E_{00}(T_0v_0)}{E_{00}(v^2_0)}<V_{00}(T_0)$$ which contradicts the fact that $T_0$ is a minimum-variance unbiased estimator of $\theta$.
>
>**(if part)** It is given that $\text{Cov }(T_0v_0)=0\;\forall\theta,\forall v\in U_0$. We have to prove that $T_0$ is an MVUE of $\theta$. Let $T$ be an estimator belonging to $U$, then $(T-T_0)\in U_0$.
>$\therefore$ From the given condition, $\text{Cov }(T_0, T_0-T)=0$
>$$\Rightarrow V(T_0)-\text{Cov }(T_0, T)=0 \Rightarrow \text{Cov }(T_0, T)=V(T_0)$$$..(1)$
>
>Now, $V(T_0-T)\geq 0$
>$$\begin{align}&\Rightarrow V(T_0)+V(T)-2\text{Cov }(T_0,T)\geq 0 \\ &\Rightarrow V(T_0)+V(T)-2V(T_0)\geq 0 &\text{from (1)}\\               &\Rightarrow V(T)\geq V(T_0) &\theta\in\Theta\end{align} $$
>
>$\because$ $T$ is an arbitrary member of $U$, the result holds

> #### Result 2 (Rao-Cramer Inequality)
> Given $X_1,X_2...X_n$ is a random sample with pdf $f(x, \theta), \theta \in \Theta$. Assume that 	$\theta$ is a non-degeerate open interval on the real line. 
> 
> Suppose $T$ is an unbiased estimator of $\gamma(\theta)$
> 
> Again assume that the joint	pdf $f(\underset{\sim}{x})=\prod_{i=1}^n f(x_i, \theta)$ satisfies the following regulatory conditions:
> 
> (a) $\frac{\delta f(\underset{\sim}{x}, \theta)}{\delta \theta}$ exists
> (b) $\frac{\delta}{\delta \theta}\int f(\underset{\sim}{x}, \theta)d \underset{\sim}{x} = \int \frac{\delta f(\underset{\sim}{x}, \theta)}{\delta \theta}d\underset{\sim}{x}$
> (c) $\frac{\delta}{\delta\theta}\int T(\underset{\sim}{x})f(\underset{\sim}{x}, \theta)d\underset{\sim}{x}=\int T(\underset{\sim}{x})\frac{\delta f(\underset{\sim}{x}, \theta)}{\delta\theta}d\underset{\sim}{x}$
> (d) $0<I(\theta)<\infty$
> 
> Where $$I(\theta)=E\left[\frac{\delta \log f(\underset{\sim}{X},\theta)}{\delta\theta}\right]^2$$ is information on $\theta$ supplied by the sample size of $n$.
> Then
> $$V(T)\geq\frac{(\gamma'(\theta))}{I(\theta)}^2 \;\; \forall\theta$$
> 
> *Proof* Since $1=\int_{R^n}f(\underset{\sim}{x}, \theta)d\underset{\sim}{x}$
> $\therefore$ We have from (b)
> $$O=\int_{R^n}\frac{\delta f\left(\underset{\sim}{x},\theta\right)}{\delta\theta}d\underset{\sim}{x}=$$

> #### Result 3 (Rao-Blackwell Theorem)
> 
