...
(3) and (4) gives us,
$$\frac{\delta^2\text{log }f(x_i,\theta)}{\delta\theta\delta x_i}\Big/\frac{\delta u(x_i)}{\delta x_i}=\frac{\delta G(T, \theta)/\delta T}{\delta G(T)/\delta T}\;\forall i$$
..(5)
...
\Rightarrow &\sum_i \frac{\delta f_i(x_i, \theta)}{\delta \theta}
\end{align}$$
### Completeness
