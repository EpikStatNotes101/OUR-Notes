...
>*e.g, Let $X~P(\lambda)$. Then $T(X)=(-2)^X$ is an unbiased estimator of $e^{-3\lambda}$ since$$E\left\{T(X)\right\}=e^\sum_x\frac{(-2\lambda)^x}{x!}=e^{-\lambda}\cdot e^{-2\lambda}=e^{-3\lambda}$$*
